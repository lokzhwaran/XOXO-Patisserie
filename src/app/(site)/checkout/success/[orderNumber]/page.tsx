...
export default async function CheckoutSuccessPage({ params }: { params: Promise<{ orderNumber: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 text-center sm:px-6 lg:px-8">
      ...
      <h1 className="mt-4 font-heading text-3xl">Order confirmed!</h1>
      <p className="mt-2 text-[var(--color-muted-foreground)]">
        Save this ID — you&apos;ll need it to track your order.
      </p>
      <CopyOrderId orderNumber={order.orderNumber} />

      <div className="mt-10 rounded-[var(--radius-lg)] border border-[var(--color-border)] p-6 text-left">
        <h2 className="font-heading text-lg">Order Summary</h2>
        <ul className="mt-3 flex flex-col divide-y divide-[var(--color-border)] text-sm">
        ...
        <div className="mt-3 flex justify-between border-t border-[var(--color-border)] pt-3 font-heading text-lg">
